...
<template>
  <div grid grid-cols-16 grid-rows-16 hw-full gap-2>
    <div v-for="item in 256" :key="item" hover="bg-light-200 text-dark-200 border-com" flex-center @click="handleClickItem">
      {{ item }}
    </div>
    <div v-if="visible" ref="popRef" h-100px w-200px overflow-y-auto transition-all scrollbar :style="{ left: `${activeShow.x}px`, top: `${activeShow.y}px` }" class="absolute rounded-md bg-light-200 shadow-md border-com">
